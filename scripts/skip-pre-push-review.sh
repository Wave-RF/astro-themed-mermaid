...
case "$name" in
  ...
  docs-reviewer)
    if { [ -x scripts/docs-prose.sh ] && [ -n "$(scripts/docs-prose.sh changed 2>/dev/null)" ]; } \
       || { [ -n "$base" ] && git diff --name-only "${base}...HEAD" 2>/dev/null | grep -qE '^(index\.d\.ts|example/)'; }; then
      warn="docs prose, index.d.ts, or example/ changed on this branch — a docs review is probably warranted"
    fi
    ;;
esac
