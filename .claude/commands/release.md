...
1. **Show what's published now:**
   ```bash
   npm view @wave-rf/astro-themed-mermaid dist-tags 2>/dev/null || echo "not yet published — see RELEASING.md for the one-time bootstrap"
   ```
...
- **Verify a published release**: `npm view @wave-rf/astro-themed-mermaid@<version>` (provenance shows on the npm page); `gh release view v<version>`.
